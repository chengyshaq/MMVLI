...
class SIMM_Model(nn.Module):
    ...
    def __init__(self, view_blocks, comm_feature_num, label_num, model_args=None):
        ...
        self.transformer = Transformer(self.final_feature_num, depth=4, num_heads=8)
    ...
    def forward(self, x, is_training=True, labels = None):
        ...
        embed_feature = final_features.unsqueeze(0)
        embed_feature = embed_feature.transpose(0, 1)
        final_features = self.transformer(embed_feature)
        final_features = final_features.transpose(0, 1).squeeze(0)
